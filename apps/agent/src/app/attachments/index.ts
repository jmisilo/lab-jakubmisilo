...
export class AttachmentService {
  static async prepareMessage(message: Message) {
    if (message.attachments.length > MAX_ATTACHMENT_COUNT) {
      throw new Error('Please send up to three attachments at a time.');
    }
    ...
    // Keep this sequential. Besides enforcing the aggregate bound before the
    // next download starts, it prevents a burst of unknown-size URLs from
    // allocating many 7 MB buffers concurrently.
