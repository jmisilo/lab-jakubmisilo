...
export const RecurringSchedulePayloadSchema = z.object({
  kind: z.literal('recurring'),
  scheduleId: z.string().min(1),
  triggerVersion: z.string().min(1).optional(),
});

export const ScheduleExecutionPayloadSchema = z.discriminatedUnion('kind', [
  OneTimeSchedulePayloadSchema,
  RecurringSchedulePayloadSchema,
]);

export type ScheduleExecutionPayload = z.infer<typeof ScheduleExecutionPayloadSchema>;
...
export const ScheduleFailureCallbackPayloadSchema = z
  .object({
    ...
    notBefore: z.coerce.number().optional(),
  })
  .passthrough();

export type ScheduleFailureCallbackPayload = z.infer<typeof ScheduleFailureCallbackPayloadSchema>;
